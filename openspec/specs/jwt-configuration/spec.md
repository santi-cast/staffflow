...
Valor comprometido eliminado: `staffflow-secret-key-para-desarrollo-minimo-32-caracteres`
(sigue en la historia de git — ver nota de deuda residual abajo).
...
- Ser diferente al valor comprometido anterior
...
El sistema DEBE documentar que el secreto literal anterior
(`staffflow-secret-key-para-desarrollo-minimo-32-caracteres`) está comprometido
por su presencia en la historia de git.
...
- GIVEN el archivo de spec canónico (`openspec/specs/jwt-configuration/spec.md`)
...
- **Secreto en historia de git**: el valor anterior sigue accesible vía `git log`. Requiere
  `git filter-repo` o BFG Repo-Cleaner en una tarea de seguimiento dedicada.
  Engram ref: implícito en la spec — no hay engram separado.
...
- **Commits**: `701c9ae` (feat externalización JWT)
- **Canonical since**: 2026-05-09
- **Verified**: 6/6 scenarios PASS
