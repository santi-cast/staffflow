 ...
 *     genérico de BD. GlobalExceptionHandler convierte
 *     IllegalStateException en HTTP 404 y DataIntegrityViolationException
 *     en HTTP 400; los duplicados se capturan aquí antes de llegar a BD.
...
public class UsuarioService {
    ...
    public List<UsuarioResponse> listar(String rol, Boolean activo) {
        ...
        if (rol != null && activo != null) {
            // Ambos filtros activos simultáneamente
     ...
     * Valida unicidad de username y email excluyendo al propio usuario que
     * se está editando (puede conservar su propio valor sin conflicto).
     *
     * Códigos HTTP producidos:
     ...
     * @param request campos a actualizar (username, email, rol — todos opcionales)
    ...
    public UsuarioResponse actualizar(Long id, UsuarioPatchRequest request) {
        ...
        // username no es modificable por este endpoint (UsuarioPatchRequest no lo incluye)
        // Solo se pueden modificar: email, rol
